...
contract TheSlugClub is ERC721, Ownable {
    ...
    function tokenLicense(uint _id) public view returns(string memory) {
        require(_id < totalSupply(), "CHOOSE A SLUG WITHIN RANGE");
        return LICENSE_TEXT;
    }
    ...
    function changeSlugName(uint _tokenId, string memory _name) public {
        require(ownerOf(_tokenId) == msg.sender, "Hey, your wallet doesn't own this slug!");
        require(sha256(bytes(_name)) != sha256(bytes(slugNames[_tokenId])), "New name is same as the current one");
        slugNames[_tokenId] = _name;
        
        emit slugNameChange(msg.sender, _tokenId, _name);
        
    }
    
    function viewSlugName(uint _tokenId) public view returns( string memory ){
        require( _tokenId < totalSupply(), "Choose a slug within range" );
        return slugNames[_tokenId];
    }
    
    
    // GET ALL Slugs OF A WALLET AS AN ARRAY OF STRINGS. WOULD BE BETTER MAYBE IF IT RETURNED A STRUCT WITH ID-NAME MATCH
    function slugNamesOfOwner(address _owner) external view returns(string[] memory ) {
        uint256 tokenCount = balanceOf(_owner);
        if (tokenCount == 0) {
            // Return an empty array
            return new string[](0);
            ...
            uint256 index;
            for (index = 0; index < tokenCount; index++) {
                result[index] = slugNames[ tokenOfOwnerByIndex(_owner, index) ] ;
            }
            return result;
        }
    }
